...
uniform float alphaDP, lambdaR, betaHS, t0HS, t1HS, noiseAmt;
vec2 readUV(vec2 p){ return texture(uState,p).rg; }
void main(){
  vec2 uv=vUv;
  vec2 UVc=readUV(uv);
  float Uc=UVc.r, Vc=UVc.g;
  ...
  float dUdx=(UVR.r-UVL.r)*0.5, dUdy=(UVT.r-UVB.r)*0.5;
  float dVdx=(UVR.g-UVL.g)*0.5, dVdy=(UVT.g-UVB.g)*0.5;
