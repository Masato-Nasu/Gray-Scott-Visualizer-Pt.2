...
let INT_FMT, PIX_FMT, PIX_TYPE;  // FBO fallback
let vao, texA, texB, texRadius, fbA, fbB;
let width, height;
let paused=false, showRadius=false;

const params = { du:0.16, dv:0.08, feed:0.035, kill:0.06, dt:1.0,
  alphaDP:0.75, lambdaR:1.0, betaHS:0.65, t0HS:0.35, t1HS:0.85, noiseAmt:0.002 };
...
function syncUI(){ ['feed','kill','du','dv','dt','alphaDP','lambdaR','betaHS','t0HS','t1HS','noiseAmt']
  .forEach(id=>{$('#'+id).value=params[id];}); updateLabels(); }

function setPresetLeopard(){ Object.assign(params,{feed:0.035,kill:0.060,alphaDP:0.40,lambdaR:0.80,betaHS:0.55,t0HS:0.30,t1HS:0.80}); syncUI(); }
function setPresetPuffer(){ Object.assign(params,{feed:0.025,kill:0.055,alphaDP:1.20,lambdaR:1.10,betaHS:0.35,t0HS:0.25,t1HS:0.70}); syncUI(); }
function setPresetZebra(){  Object.assign(params,{feed:0.037,kill:0.064,alphaDP:0.50,lambdaR:0.70,betaHS:0.45,t0HS:0.28,t1HS:0.80}); syncUI(); }
function setPresetMosaic(){ Object.assign(params,{feed:0.020,kill:0.050,alphaDP:1.40,lambdaR:1.30,betaHS:0.70,t0HS:0.35,t1HS:0.95}); syncUI(); }

function createGL(){
  gl = canvas.getContext('webgl2',{antialias:false,preserveDrawingBuffer:true});
  if(!gl){ alert('WebGL2 非対応です'); throw new Error('no webgl2'); }
  const extFloat = gl.getExtension('EXT_color_buffer_float');
  const extHalf  = gl.getExtension('EXT_color_buffer_half_float');
  if(extFloat){ INT_FMT=gl.RGBA32F; PIX_TYPE=gl.FLOAT; }
  else if(extHalf){ INT_FMT=gl.RGBA16F; PIX_TYPE=gl.HALF_FLOAT; }
  else { INT_FMT=gl.RGBA8; PIX_TYPE=gl.UNSIGNED_BYTE; }
  PIX_FMT=gl.RGBA;
}
...
async function createPrograms(){
  ...
  const fsS=compile(gl.FRAGMENT_SHADER,simSrc);
  const fsV=compile(gl.FRAGMENT_SHADER,visSrc);
  ...
  if(!gl.getProgramParameter(progSim,gl.LINK_STATUS)){ console.error(gl.getProgramInfoLog(progSim)); throw new Error('link sim'); }
  ...
  if(!gl.getProgramParameter(progVis,gl.LINK_STATUS)){ console.error(gl.getProgramInfoLog(progVis)); throw new Error('link vis'); }
}
...
function createFBO(tex){
  ...
  if(status!==gl.FRAMEBUFFER_COMPLETE){ throw new Error('FBO incomplete'); }
  return fb;
}
...
function makeRadiusTex(w,h,mean=0.5,std=0.18){
  ...
  for(let i=0;i<w*h;i++){
    ...
    let r=mean+std*g; r=Math.min(1,Math.max(0,r)); rarr[i]=r;
  }
  const out=new Float32Array(w*h);
  for(let y=0;y<h;y++){ for(let x=0;x<w;x++){
      let s=0,c=0;
      for(let j=-1;j<=1;j++){ for(let i=-1;i<=1;i++){
        const xx=(x+i+w)%w, yy=(y+j+h)%h; s+=rarr[yy*w+xx]; c++;
      }}
      out[y*w+x]=s/c;
  }}
  for(let i=0;i<w*h;i++){ const r=out[i]; data[i*4+0]=r; data[i*4+1]=r; data[i*4+2]=r; data[i*4+3]=1; }
...
function seedCenter(){
  ...
  for(let y=-s;y<=s;y++){ for(let x=-s;x<=s;x++){
    const cx=Math.floor(width/2)+x, cy=Math.floor(height/2)+y;
    if(cx<0||cy<0||cx>=width||cy>=height) continue;
    const idx=(cy*width+cx)*4; init[idx+0]=0.50; init[idx+1]=0.25;
  }}
...
function bindUI(){
  ['feed','kill','du','dv','dt','alphaDP','lambdaR','betaHS','t0HS','t1HS','noiseAmt'].forEach(id=>{
    $('#'+id).addEventListener('input',(e)=>{ params[id]=parseFloat(e.target.value); updateLabels(); });
  });
